...
Eigen::MatrixXd Mat_Import(const std::string& filename)
{
...
int main() {
    ...
    //initing loaded matrices in eigen
    Eigen::MatrixXd Aineq = Mat_Import("../Aineq.bin");
    Eigen::MatrixXd Bineq = Mat_Import("../Bineq.bin");
    Eigen::MatrixXd QQ    = Mat_Import("../QQ.bin");
    Eigen::MatrixXd H1    = Mat_Import("../H1.bin");

    //dimension since A and B are not in this file
    int dim = Aineq.rows()/2;
    ...
    while (true) {
        ...
        quadprogpp::Matrix<double> CE(dim,0); //no inequality constraints
        ...
        quadprogpp::Vector<double> xqp(dim);
        //making x for qp all 0.0 without eigen
        for(int i=0;i<dim;i++) xqp[i] = 0.0;
