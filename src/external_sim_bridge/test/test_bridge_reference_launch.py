...
def _load_reference_trajectory():
    with open(REFERENCE_FIXTURE_PATH, newline='', encoding='utf-8') as csv_file:
...
class TrajectoryRecorder(Node):
    def __init__(self):
        super().__init__('external_bridge_trajectory_recorder')
        self.trajectory = []
    ...
    return (
        launch.LaunchDescription([
            ...
            TimerAction(
                ...
                actions=[EmitEvent(event=Shutdown(reason='bridge trajectory capture complete'))],
            ),
        ]),
        {},
    )
        ...
        for step, (actual_row, expected_row) in enumerate(
            ...
            for index, (actual_value, expected_value) in enumerate(
                ...
                self.assertLessEqual(
                    difference,
                    ABS_TOLERANCE,
                    f'Bridge trajectory mismatch at step '
...
class TestBridgeLaunchShutdown(unittest.TestCase):
