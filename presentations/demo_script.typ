...
This is also a validated baseline, not just a visual demo. This closed-loop ROS 2 path has unit tests and a regression test that compares the trajectory it produces against the original reference implementation. So what we are showing live is the same baseline we can also verify in software.
