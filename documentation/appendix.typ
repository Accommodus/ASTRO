...
Both machines must have Docker installed and a valid Tailscale auth key. If the host does not expose `/dev/net/tun`, set `TS_USERSPACE=true` to use Tailscale's userspace networking mode.
...
Each profile starts a Tailscale sidecar alongside the satellite container, sharing the sidecar's network namespace via `network_mode: service:tailscale-*`. The entrypoint auto-detects peer hostnames via Tailscale DNS when `ROS_AUTO_TAILSCALE_PEER=1`.
